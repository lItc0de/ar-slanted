...
const initArToolkit = async () => {
  ...
  await markers.reduce(async (prev, marker) => {
    ...
    // ar.addNFTMarker(ar.id, markerUrl, (nftMarker) => {
    //   console.log('markerID:', nftMarker.id);
    //   ar.trackNFTMarkerId(nftMarker.id, nftMarker.width);
    // });
...
const startProcess = (controllers, video, canvas) => {
  ...
  const process = () => {
    ...
    const ar = controllers[0];
    const result = ar.detectMarker(imageData);
    console.log(result);
    ar.detectNFTMarker();
    const nftMarkerInfo = ar.getNFTMarker(result);
    console.log('info', nftMarkerInfo);
